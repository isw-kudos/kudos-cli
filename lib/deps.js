...
function ensureKudosFolder(folder, filename) {
  if (!filename.startsWith('kudos-')) return Promise.resolve(null);
    ...
    .then(APPS =>
      Promise.all(
        [CORE, ...APPS].map(target => {
          return getKudosDependenciesIn(target).then(apps =>
            apps.forEach(app => {
              config[app] = config[app] || {
                path: getBuildPath(app),
                targets: [],
              };
              config[app].targets.push(target);

              getKudosDependenciesIn(`${target}/node_modules/${app}`).then(
                secondLevel => {
                  if (secondLevel && secondLevel.length) {
                    console.error('\n!!! NESTED DEPENDENCY DETECTED !!!');
                    console.error(
                      `Please check "${target}/node_modules/${app}/node_modules" as it has shouldnt have: ${secondLevel}\n`
                    );
                    console.error(
                      'This is normally caused by different versions (commit hash) in package.lock'
                    );
                    console.error(
                      'Easiest solution is to delete the package.lock and the node_modules and npm i \n'
                    );
                  }
                }
              );
